...
ALTER TABLE finance_invoices ADD COLUMN IF NOT EXISTS document_snapshot TEXT;
ALTER TABLE finance_invoices ADD COLUMN IF NOT EXISTS revision INTEGER NOT NULL DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS finance_invoice_revisions (
 id BIGSERIAL PRIMARY KEY, business_id INTEGER NOT NULL, invoice_id INTEGER NOT NULL,
...
UPDATE finance_invoices SET document_snapshot=(SELECT json_build_object(
 'sender',json_build_object('name',b.business_name,'address',COALESCE(p.address,''),'phone',COALESCE(p.business_phone,''),'email','','tax_id','','website',''),
 'recipient',json_build_object('name',c.name,'pic','','address','','phone',COALESCE(c.phone,''),'email',COALESCE(c.email,''),'tax_id',''),
 'payment',json_build_object('method',CASE WHEN COALESCE(p.payment_bank_name,'')<>'' THEN 'Transfer Bank' ELSE '' END,'bank',COALESCE(p.payment_bank_name,''),'account_number',COALESCE(p.payment_account_number,''),'account_holder',COALESCE(p.payment_account_name,''),'instructions',COALESCE(p.payment_instructions,'')),
 'reference','')::text
