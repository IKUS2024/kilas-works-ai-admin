...
SLOTS=('name','phone','email','notes','amount','currency','date','description','account','category','project','customer',
       ...
       'settlement','target','customer_reference','target_reference','group_by','branch','overdue','aging','due_week','page')
...
def visible_options(b,u):
    ...
    return {key:[{k:str(row[k])[:200] for k in ('name','title','invoice_number','currency','direction') if k in row}
                 for row in rows[:50]] for key,rows in pools.items()}
...
def last_record(b,u,previous):
    ...
    if ref.get('kind') in ('transaction','invoice','recurring','account','category','branch','fx'):
        ...
        return (ref['kind'],row) if row else (None,None)
    return None,None
...
def safe_context(b,u,previous,context=None,current=None):
    ...
    if previous.get('plan'):
        state['previous_query']={k:v for k,v in previous['plan'].items() if k not in ('transaction','exchange')}
...
def read(b,u,intent,slots,previous):
    ...
    if 'customer_reference' in slots:
        kind,row=last_record(b,u,previous)
        if kind!='customer':return uncertain()
        p['customer']=row['name']
    if 'period' in slots:
...
def apply_slots(b,u,initial,slots,previous):
    ...
    context['conversation']={k:previous[k] for k in ('last_record','plan') if k in previous}
    slots=dict(slots)
    if 'customer_reference' in slots:
        kind,row=last_record(b,u,previous)
        if kind!='customer' or 'customer_id' not in context['values']:return uncertain()
        context['values']['customer_id']=str(row['id']);slots.pop('customer_reference')
    settlement=slots.pop('settlement',None)
    if settlement and (context['action']!='record_invoice_payment' or settlement.casefold() not in ('lunas','lunasi','pelunasan','full payment','pay in full')):return uncertain()
    if settlement:context['text']='lunas'
    ...
    try:values=draft.resolve(slots,context,initial['fields'])
        ...
        return result
    if settlement and values.get('invoice_id') and not values.get('amount'):
        invoice=flow.f.get_finance_invoice(b,int(values['invoice_id']),u)
        outstanding=flow.f.get_invoice_totals(b,invoice['id'],u)['outstanding_minor']
        values['amount']=str(flow.fx.major(outstanding,invoice['currency']))
...
def start(b,u,intent,slots,previous):
    ...
    if intent in commands.TITLES or intent=='issue_invoice':
        ...
        elif intent=='record_invoice_payment':values['invoice_id']=''
...
def pending(b,u,message,context,current,query_context=''):
    ...
    if updates:return None,updates,True
    ...
    if intent=='continue_draft':
        ...
        if 'customer_reference' in slots:
            kind,row=last_record(b,u,previous)
            if kind!='customer' or 'customer_id' not in context['values']:return uncertain(),{},False
