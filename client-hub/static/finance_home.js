(() => {
  ...
  const select = document.getElementById('finance-trend-currency');
  if (!data || !chart || !select) return;
  ...
  function render() {
    const items = rows.filter(row => row.currency === select.value);
    const max = Math.max(1, ...items.flatMap(row => [row.income_minor, row.expense_minor]));
    const svg = node('svg', {viewBox: '0 0 480 200', role: 'img', 'aria-label': `Pemasukan dan pengeluaran enam bulan dalam ${select.value}. Rincian tersedia di bawah grafik.`});
    const format = value => new Intl.NumberFormat('id-ID', {notation:'compact', maximumFractionDigits:1}).format(value / (['IDR', 'JPY'].includes(select.value) ? 1 : 100));
    [0, .5, 1].forEach(level => {
    ...
    items.forEach((row, index) => {
      ...
      ['income_minor', 'expense_minor'].forEach((key, j) => {
        ...
        rect.append(node('title', {}, `${row.month} ${j ? 'Pengeluaran' : 'Pemasukan'}: ${row[key] / (['IDR', 'JPY'].includes(row.currency) ? 1 : 100)} ${row.currency}`));
        svg.append(rect);
      });
    ...
    document.getElementById('finance-trend-empty').hidden = items.some(row => row.income_minor || row.expense_minor);
  }
