...
(() => {
  ...
  async function complete() {
    if (!started || sent || !code || !assets) return;
    sent = true;
    ...
    try {
      const response = await fetch(config.endpoint, {
        ...
        body: JSON.stringify({state: config.state, code, ...assets})
      });
      code = assets = null;
  ...
  window.addEventListener('message', event => {
    ...
    try {
      ...
      // This release supports the standard Cloud API flow, not coexistence/migration variants.
      if (result.event === 'FINISH' && result.data && !result.data.is_wa_login_user && result.data.waba_id && result.data.phone_number_id) {
        assets = {waba_id: String(result.data.waba_id), phone_number_id: String(result.data.phone_number_id)};
        complete();
      } else if (result.event === 'CANCEL' || result.event === 'ERROR' || String(result.event).startsWith('FINISH')) failed();
  ...
  button.addEventListener('click', () => {
    ...
    FB.login(response => {
      ...
      complete();
        ...
        extras: {} // v4: product selection belongs to the Facebook Login for Business configuration.
       });
  });
