...
def export_csv(name,business_id,filters,actor_user_id):
    ...
    if name=='transactions':
        headers=('tanggal','jenis','nominal_rupiah','akun','kategori','customer','proyek','pihak_lawan','deskripsi','status','sumber')
        rows=((r['occurred_on'],DIRECTIONS[r['direction']],r['amount_minor'],r['account_name'],r['category_name'],r['customer_name'],r['project_name'],r['counterparty_name'],r['description'],STATUSES[r['status']],SOURCES.get(r['source_type'],'Manual')) for r in data)
    elif name=='invoices':
        headers=('nomor_invoice','customer','tanggal_terbit','jatuh_tempo','status','total_rupiah','sudah_dibayar_rupiah','sisa_rupiah','hari_terlambat','overdue')
        rows=((r['invoice_number'],r['customer_name'],r['issue_date'],r['due_date'],STATUSES[r['status']],r['total_minor'],r['paid_minor'],r['outstanding_minor'],r['days_late'],'Ya' if r['overdue'] else 'Tidak') for r in data)
        ...
        headers=('akun','jenis','status','saldo_awal_rupiah','pemasukan_rupiah','pengeluaran_rupiah','saldo_rupiah')
        rows=((r['name'],ACCOUNT_TYPES[r['account_type']],'Aktif' if r['is_active'] else 'Nonaktif',r['opening_balance_minor'],r['income_minor'],r['expense_minor'],r['balance_minor']) for r in data)
        ...
        headers=('biaya_rutin','tanggal_jatuh_tempo','nominal_rupiah','proyek','akun','kategori')
        rows=((r['name'],r['scheduled_on'],r['amount_minor'],r['project_name'],r['account_name'],r['category_name']) for r in data)
