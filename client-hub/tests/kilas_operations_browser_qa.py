...
def main():
    with sync_playwright() as p:
        ...
        expect(visitor.get_by_text('Tim siap membantu pengiriman Anda.',exact=True)).to_be_visible(timeout=15000)
        ...
        expect(visitor.get_by_text(FOLLOWUP,exact=True)).to_have_count(1,timeout=15000)
        run(owner)
        expect(visitor.get_by_text(FOLLOWUP,exact=True)).to_have_count(1)
        ...
        run(owner)
        expect(visitor.get_by_text(REVIEW,exact=True)).to_have_count(1,timeout=15000)
        run(owner)
        expect(visitor.get_by_text(REVIEW,exact=True)).to_have_count(1)
