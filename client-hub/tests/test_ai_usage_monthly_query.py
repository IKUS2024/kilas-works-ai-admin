...
class MonthlyQueryTests(unittest.TestCase):
    ...
    def test_sqlite_results_match_original_like_semantics(self):
        ...
        self.assertTrue(usage.record('claude-haiku-4-5-20251001',response,tenant_id=self.bid,context='finance_chat'))
        self.assertTrue(usage.record('claude-sonnet-4-6',response,tenant_id=self.bid,context='finance_receipt',classification='vision'))
        self.assertTrue(usage.record('claude-sonnet-4-6',response,tenant_id=self.bid,context='finance_bank',classification='vision'))
        self.assertTrue(usage.record('claude-sonnet-4-6',response,tenant_id=self.bid,context='finance_document',classification='vision'))
        self.assertTrue(usage.record('claude-haiku-4-5-20251001',response,tenant_id=self.bid,context='finance_analyst'))
        self.assertTrue(usage.record('claude-haiku-4-5-20251001',response,tenant_id=self.bid,context='finance_operator'))
        ...
        self.assertEqual((row['calls'],row['haiku_calls'],row['sonnet_calls']),(11,6,4))
        self.assertEqual((row['finance_calls'],row['finance_vision_calls']),(7,3))
        self.assertEqual((row['finance_chat_calls'],row['finance_receipt_calls'],row['finance_bank_calls']),
                         (1,1,1))
        self.assertEqual((row['finance_document_calls'],row['finance_analyst_calls'],row['finance_operator_calls'],row['finance_other_calls']),
                         (1,1,1,1))
