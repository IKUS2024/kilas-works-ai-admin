...
def main():
    with sync_playwright() as p:
        ...
        expect(owner.get_by_role('heading',name='Buat Job')).to_be_visible()
        owner.get_by_label('Judul',exact=True).fill('Pesanan kantor Jumat')
        owner.get_by_label('Catatan singkat',exact=True).fill('Siapkan 20 paket makan siang')
        ...
        fits(owner)
        owner.get_by_role('button',name='Buat Job',exact=True).click()
        ...
        owner.get_by_label('Cari Job',exact=True).fill('kantor')
        owner.get_by_label('Status',exact=True).select_option('NEEDS_INFORMATION')
