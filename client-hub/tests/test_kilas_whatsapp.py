...
class WhatsAppTests(unittest.TestCase):
    ...
    def test_owner_inbox_scope_channel_and_csrf(self):
        ...
        self.assertEqual(response.status_code,200);self.assertIn(b'WhatsApp',response.data)
        # WhatsApp contacts start as Leads. Jobs is Customer-only, so the old linked-Job
        # details (e.g. origin "Guangzhou") must not leak into the Lead Inbox surface.
